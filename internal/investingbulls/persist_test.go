...
func (m *memoryLearnStore) RecentCandles(context.Context, string, string, int) ([]domain.Kline, error) { return m.candles, nil }
func (m *memoryLearnStore) UpsertStrategy(_ context.Context, s domain.Strategy) error {
	if m.strategies == nil { m.strategies = map[string]domain.Strategy{} }
	m.strategies[s.ID] = s
	return nil
}
func (m *memoryLearnStore) GetStrategy(_ context.Context, id string) (domain.Strategy, error) {
	s, ok := m.strategies[id]
	if !ok { return domain.Strategy{}, domain.ErrNotFound }
	return s, nil
}
...
func (m *memoryLearnStore) LastBacktest(_ context.Context, string) (domain.BacktestResult, error) {
	if len(m.backtests) == 0 { return domain.BacktestResult{}, domain.ErrNotFound }
...
func TestLearnAndPersistRejectsInsufficientHistory(t *testing.T) {
	...
	if err == nil { t.Fatal("expected insufficient history error") }
}
...
func TestLearnAndPersistRequiresStore(t *testing.T) {
	...
	if err == nil { t.Fatal("expected store error") }
}
