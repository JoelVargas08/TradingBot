...
func EvaluateLive(ctx context.Context, store domain.CandleStore, strategy domain.Strategy, k domain.Kline) (domain.SignalEvent, bool, error) {
    var model MultiTimeframeModel
    if err:=json.Unmarshal([]byte(strategy.Spec),&model);err!=nil{return domain.SignalEvent{},false,fmt.Errorf("investing bulls spec: %w",err)}
    if model.Timeframes.EntryTimeframe=="" {return domain.SignalEvent{},false,fmt.Errorf("investing bulls spec: entry timeframe vacío")}
    if k.Timeframe!=model.Timeframes.EntryTimeframe {return domain.SignalEvent{},false,nil}
    main,err:=store.RecentCandles(ctx,k.Symbol,model.Timeframes.MainTimeframe,5000);if err!=nil{return domain.SignalEvent{},false,err}
    ...
    setups:=EvaluateConfluence(ep,es,fib,imbs,blocks,model.Base.Confluence);valid:=false;var matched Setup
    for _,s:=range setups{if s.Index==len(ep)-1&&s.Direction==candidate.Direction&&s.Valid{matched=s;valid=true;break}}
    if !valid{return domain.SignalEvent{},false,nil}
    plan,ok:=buildLearningPlan(matched,fib,k.Close,es,blocks,model.Base.TradePlan);if !ok{return domain.SignalEvent{},false,nil}
